...
masyu = Drawers p (solstyle . drawEdges . snd <> p . fst)
  where p = placeGrid . fmap pearl . clues <> grid gDefault

nurikabe :: Backend' b => Drawers b (Grid C (Maybe Int)) ShadedGrid
nurikabe = Drawers drawIntGrid (drawIntGrid . fst <> drawShade . snd)

latintapa
  :: Backend' b => Drawers b (Grid C (Maybe [String])) (Grid C (Maybe Char))
latintapa = Drawers l (l . fst <> placeGrid . fmap drawChar . clues . snd)
  where l = grid gDefault <> drawWordsClues

sudoku :: Backend' b => Drawers b (Grid C (Maybe Int)) (Grid C (Maybe Int))
sudoku = Drawers (placeGrid . fmap drawInt . clues <> sudokugrid)
...
killersudoku = Drawers (p <> placeGrid . fmap drawInt . clues . trd3)
...
pyramid = Drawers DPyr.pyramid (DPyr.pyramid . merge)
  where merge (p, q) = Pyr.mergepyramidsol p q
...
slither =
  Drawers drawSlitherGrid (drawSlitherGrid . fst <> solstyle . drawEdges . snd)
...
tightfitskyscrapers = Drawers
  ...
  .  fmap (fmap drawInt)
  .  fst
  <> drawTightGrid (const mempty)
  .  snd
  )
  (placeOutside . fmap (fmap drawInt) . fst . fst <> drawTightGrid drawInt . snd
  )
...
wordloop = Drawers (uncurry wordgrid) (drawCharGrid . snd)
...
wordsearch = Drawers
  (uncurry wordgrid)
  (solstyle . drawMarkedWords . snd . snd <> drawCharGrid . fst . snd)

curvedata :: Backend' b => Drawers b (Grid C (Maybe [Edge N])) [Edge C]
curvedata = Drawers
  (placeGrid . fmap drawCurve . clues <> grid gDefault)
  (  placeGrid
  .  fmap drawCurve
  .  clues
  .  fst
...
tapa = Drawers tapaGrid (tapaGrid . fst <> drawShade . snd)
  where tapaGrid = placeGrid . fmap drawTapaClue . clues <> grid gDefault

japanesesums
  :: Backend' b
  => Drawers b (OutsideClues C [Int], String) (Grid C (Either Black Int))
japanesesums = Drawers (outsideIntGrid . fst <> n)
...
coral =
  Drawers drawMultiOutsideGrid (drawMultiOutsideGrid . fst <> drawShade . snd)
...
labyrinth = Drawers (placeGrid . fmap drawInt . clues . fst3 <> p <> n)
...
bahnhof = Drawers
  ...
  where drawBahnhofStation = either drawInt (const mempty)
...
anglers = Drawers (p <> g) (p . fst <> solstyle . drawEdges . snd <> g . fst)
 where
  p =
    placeOutside
      .  fmap (fmap drawInt')
...
skyscrapers = Drawers
 ...
 where
  g = placeOutside . fmap (fmap drawInt) <> grid gDefault . outsideGrid
  n (oc, s) = placeNoteTR (outsideSize oc) (text' s)

shikaku :: Backend' b => Drawers b (Grid C (Maybe Int)) AreaGrid
shikaku = Drawers p (drawAreas . snd <> p . fst)
  where p = placeGrid . fmap drawInt . clues <> grid gDashed
...
skyscrapersStars = Drawers
 ...
 where
  g = (placeOutside . fmap (fmap drawInt) <> grid gDefault . outsideGrid) . fst
  n (oc, s) =
    placeNoteTR (outsideSize oc) (drawInt s ||| strutX' 0.2 ||| drawStar Star)
...
summon
  ...
  => Drawers
       ...
       (Grid C (Maybe Int))
summon = Drawers (p <> n) (placeGrid . fmap drawInt . clues . snd <> p . fst)
 where
  p (g, oc, _) =
    grid gDefault g
      <> drawAreasGray g
      <> (placeOutside . al . fmap (fmap (scale 0.7 . drawInt)) $ oc)
  al
    :: Backend' b
...
buchstabensalat = Drawers
 ...
 where
  p =
    (placeOutside . fmap (fmap drawChar) <> grid gDefault . outsideGrid) . fst
  n (ocs, ls) = placeNoteTR (outsideSize ocs) (text' ls # scale noteScale)
...
doppelblock = Drawers (p <> n) (p . fst <> placeGrid . fmap drawVal . snd)
 where
  p = drawOutsideGrid . fmap (fmap show)
...
sudokuDoppelblock = Drawers p (p . fst <> placeGrid . fmap drawVal . snd)
 where
  p =
    placeOutside
      .  fmap (fmap (scale outsideScale . drawInt))
      .  snd
...
dominos = Drawers
 ...
 where
  p (g, r) = ((placeGrid . fmap drawInt . clues <> grid gDashed) $ g)
    `aboveT'` drawDominos r
 ...
 where
  ...
  p (g, ds, ps) =
    ((placeGrid . fmap drawInt . clues <> grid gDashed) $ g)
      `aboveT'` (drawDominos ds ||| strutX' 0.5 ||| drawPills ps)
...
numberlink = Drawers
  ...
  where drawInt' x = drawInt x <> draw (square 0.7 # lc white # fc white)

loopki :: Backend' b => Drawers b (Grid C (Maybe MasyuPearl)) (Loop N)
loopki = Drawers p (solstyle . drawEdges . snd <> p . fst)
  where p = placeGrid . fmap (scale 0.5 . pearl) . clues <> grid gSlither
...
starbattle = Drawers
 ...
 where
  ...
  n (g, k) =
    placeNoteTR (size' g) (drawInt k ||| strutX' 0.2 ||| drawStar Star)
  size' = size . Map.mapKeys toCoord

heyawake :: Backend' b => Drawers b (AreaGrid, Grid C (Maybe Int)) (Grid C Bool)
heyawake = Drawers (as <> cs) (as . fst <> drawShade . snd <> cs . fst)
 where
...
pentominous = Drawers
  ...
  (placeGrid . fmap drawChar . clues . fst <> (drawAreas <> grid gDashed) . snd)
...
kropki = Drawers (p <> n) (placeGrid . fmap drawInt . snd <> p . fst)
 where
  p =
    placeGrid'
...
tomTom = Drawers p (placeGrid . fmap drawInt . snd <> p . fst)
 where
  p =
    ((drawAreas <> grid gDashed) . fst <> placeGrid . fmap hintTL . clues . snd)

horseSnake
  :: Backend' b => Drawers b (Grid C (Maybe (Either MEnd Int))) [Edge C]
horseSnake = Drawers p (solstyle . drawEdges . snd <> p . fst)
 where
...
illumination = Drawers
 ...
 where
  p = placeOutside . fmap (fmap drawFraction) <> grid gDashed . outsideGrid
...
galaxies
  ...
  => Drawers
       ...
       AreaGrid
galaxies = Drawers p (p . fst <> drawAreas . snd)
 where
...
mines = Drawers
 ...
 where
  p =
    grid gDefault
      <> placeGrid
      .  fmap (\i -> drawInt i <> fillBG lightgray)
      .  clues
...
tents = Drawers p (p . fst <> placeGrid . fmap drawTent . clues . snd)
 where
  p =
    placeOutside
      .  fmap (fmap drawInt)
...
pentominoSums = Drawers p (solgrid ||| const (strutX' 1.0) ||| table)
 where
  p (ocs, ds) =
    (         (   (drawMultiOutsideGrid ocs <> n (ocs, ds))
              ||| strutX' 1.0
              ||| emptyTable ocs
              )
  ...
  solgrid =
    skel . fst3 . snd <> drawMultiOutsideGrid . trd3 . snd <> cells . fst3 . snd
  fst3 (x, _, _) = x
  trd3 (_, _, z) = z
...
coralLits = Drawers
  ...
  where skeletonStyle = lc white . lwG (3 * onepix)
...
snake = Drawers p s
 where
  ...
  p  = cs <> placeGrid . fmap drawBigEnd . clues . snd <> grid gDefault . snd
  s =
    cs
      .  fst
      <> grid gDefault
