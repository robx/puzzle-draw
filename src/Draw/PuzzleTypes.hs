...
lits = Drawers (grid gDefault <> areasGray)
...
latintapa = Drawers l (l . fst <> placeGrid . fmap char . clues . snd)
  where l = grid gDefault <> wordsClues

sudoku :: Backend' b => Drawers b (Grid C (Maybe Int)) (Grid C (Maybe Int))
sudoku = Drawers (placeGrid . fmap int . clues <> sudokugrid)
...
killersudoku = Drawers (p <> placeGrid . fmap int . clues . trd3)
...
tapa = Drawers tapaGrid (tapaGrid . fst <> shade . snd)
  where tapaGrid = placeGrid . fmap tapaClue . clues <> grid gDefault
...
maximallengths = Drawers g (solstyle . edges . snd <> g . fst)
  where g = placeOutside . fmap (fmap int) <> grid gDefault . Data.outsideGrid

labyrinth
  :: Backend' b
  => Drawers b (Grid C (Maybe Int), [Edge N], String) (Grid C (Maybe Int))
labyrinth = Drawers (placeGrid . fmap int . clues . fst3 <> p <> n)
...
bahnhof = Drawers
  ...
  where bahnhofStation = either int (const mempty)
...
blackoutDominos = Drawers
 ...
 where
  p (g, ds) = (placeGrid . fmap int . clues <> grid gDashedThick $ g)
...
shikaku = Drawers p (areas . snd <> p . fst)
  where p = placeGrid . fmap int . clues <> grid gDashed
...
numberlink = Drawers
  ...
  where int' x = int x <> draw (square 0.7 # lc white # fc white)
...
pentopia = Drawers p (p . fst <> shade . snd)
  where p = placeGrid . fmap myopia . clues <> grid gDefault
