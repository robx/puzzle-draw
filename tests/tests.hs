...
import           Parse.Puzzle
import           Data.Elements                  ( Thermometer
                                                , MasyuPearl(..)
                                                )
import           Parse.Util                     ( parseChar
...
test_multioutside = Right oc @=? res
 where
  ...
  oc =
    OC [[3], [1, 2]] [[1, 0], []] [[0, 0, 1]] [[1, -1]] :: OutsideClues
        Coord
        [Int]
...
parseDataTests = testGroup
  ...
  $  test_pyramid_sol
  @? "wrong solution"
  , testCase "parse thermos" $ testThermo
    test_thermo_1
