...
listExamples =
    Http.get "/examples" (Json.list decodeExample)
...
render output body =
  ...
  Http.request
    { method = "POST"
    , headers = []
    , url = "http://localhost:8000/puzzle?output=" ++ out
...
view model =
    ...
    , body =
        ...
        , Html.div [] <|
          [ Html.form [ Attr.method "post", Attr.id "downloadForm", Attr.action "/download" ] <|
            let format f = [ Html.input [ Attr.id <| "o" ++ f
                                        , Attr.type_ "radio"
                                        , Attr.name "format"
                                        , Attr.value f
        ...
        , Html.div [] <|
           let updating =              model.renderState == Rendering || model.renderState == Queued
           in
           [ Html.div [] [ Html.text <| "Preview" ++ if updating then " (updating...)" else "" ]
           , 
             case model.image of
                SVG svg ->
    ...
    let
        rerender m =
            case m.renderState of
                Ready -> ({ m | renderState = Rendering }, Http.send RenderResult (render m.output m.puzzle))
                Rendering -> ({ m | renderState = Queued }, Cmd.none)
                Queued -> (m, Cmd.none)
    in
    case msg of
        PuzzleChange puzzle -> rerender { model | puzzle = puzzle }
        OutputChange outstr ->
            let output =  case outstr of
        ...
        ExamplesChange path -> ( model, Http.send ExampleResult (loadExample path) )
        RenderResult res ->
           let m2 = case res of
                      Err error -> { model | image = Error (Debug.toString error) }
                      Ok svg -> { model | image = SVG svg }
           in
             case model.renderState of
                 Ready -> (m2, Cmd.none)
                 Rendering -> ( {m2 | renderState = Ready }, Cmd.none )
                 Queued -> rerender { m2 | renderState = Ready } 
        ExamplesResult res -> case res of
           Err error -> (model, Cmd.none)
           Ok examples -> ({ model | examples = examples }, Cmd.none )
        ExampleResult res -> case res of
           Err error -> (model, Cmd.none)
           Ok example -> rerender { model | puzzle = example }
